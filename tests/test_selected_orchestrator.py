...
from amplifier_runtime.kernel.config import _foundation_load_source, preserve_selected_orchestrator
...
async def test_selected_loop_survives_app_foundation_overlay(tmp_path: Path) -> None:
    ...
    root = await load_bundle(_foundation_load_source(str(tmp_path / "profile.yaml")))
    overlay = await load_bundle(_foundation_load_source(str(tmp_path / "app.yaml")))
...
async def test_inherited_loop_remains_replaceable(tmp_path: Path) -> None:
    ...
    profile.write_text(
        f"bundle:\n  name: profile\nincludes:\n  - bundle: {_foundation_load_source(str(base))}\n"
    )
    overlay = tmp_path / "app.yaml"
    overlay.write_text("bundle:\n  name: app\nsession:\n  orchestrator:\n    module: app-loop\n")
    root = await load_bundle(_foundation_load_source(str(profile)))
    composed = root.compose(await load_bundle(_foundation_load_source(str(overlay))))
    preserve_selected_orchestrator(root, composed)
...
async def test_same_loop_overlay_config_still_wins(tmp_path: Path) -> None:
    ...
    root = await load_bundle(_foundation_load_source(str(root_path)))
    composed = root.compose(await load_bundle(_foundation_load_source(str(overlay))))
    preserve_selected_orchestrator(root, composed)
